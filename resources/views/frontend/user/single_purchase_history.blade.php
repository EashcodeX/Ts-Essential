@foreach($orders as $order)
    <div class="mb-4">
        <div class="d-flex justify-content-between align-items-center mb-2">
            <div>
                <p class="mb-2 fs-16 fw-700 deep-blue mb-0"><a class="deep-blue"
                        href="{{route('purchase_history.details', encrypt($order->id))}}">{{ translate('Order ID')}} -
                        {{ $order->code }}</a></p>
                <span class="text-muted d-block d-md-none fs-12">{{ translate('Date')}}:
                    {{ date('d-m-Y', $order->date) }}</span>
            </div>
            ...
            <div class="d-flex gap-2 d-md-none">
                ...
                <a href="{{ route('orders.track', ['order_code' => $order->code]) }}"
                    class="btn btn-sm border rounded px-4 py-1 text-muted ml-2">
                    {{ translate('Track') }}
                </a>

                <div class="dropdown">
                    <button type="button" class="btn btn-sm dropdown-toggle text-white px-4 py-1 rounded btn-options ml-2"
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                        <a class="dropdown-item text-secondary dropdown-bg-hover"
                            href="{{route('purchase_history.details', encrypt($order->id))}}"><i
                                class="las la-eye mr-2"></i>{{ translate('View') }}</a>
                        <a class="dropdown-item text-secondary dropdown-bg-hover"
                            href="{{ route('invoice.download', $order->id) }}"><i
                                class="las la-download mr-2"></i>{{ translate('Invoice') }}</a>
                        @if ($order->delivery_status == 'pending' && $order->payment_status == 'unpaid')
                            <a href="javascript:void(0)" class="dropdown-item text-secondary dropdown-bg-hover confirm-delete"
                                data-href="{{route('purchase_history.destroy', $order->id)}}"><i class="las la-trash mr-2"></i>
                                {{ translate('Cancel') }}</a>
                        @endif
                    </div>
                </div>
        ...
        <div class="row align-items-center mb-2 d-none d-md-flex">
            ...
            <div class="col-md-6 text-right">
                ...
                <a href="{{ route('orders.track', ['order_code' => $order->code]) }}"
                    class="btn btn-sm border rounded px-4 py-1 text-muted ml-1">
                    {{ translate('Track') }}
                </a>
