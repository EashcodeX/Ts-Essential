...
@if(Auth::check())
    @foreach (Auth::user()->addresses as $key => $address)
        @php
            ...
            $city_status = $city->status ?? 0;
            $active_area_exists = $city->id ? $city->areas()->where('status', 1)->exists() : false;
            ...
            <div class="row">
                <div class="col-md-8">
                    <label class="aiz-megabox d-block bg-white mb-0">
                        <input type="radio" name="address_id" value="{{ $address->id }}" {{ $address->id == $address_id && !$is_disabled ? 'checked' : '' }} {{ $is_disabled ? 'disabled' : '' }} required>
