...
@section('content')
    ...
    <div class="aiz-titlebar text-left mt-2 mb-3">
        <div class="row align-items-center">
            ...
            <div class="col-md-6 text-md-right">
                <div class="bg-white border d-inline-block py-1 px-3 rounded text-left">
                    <div class="fs-10 text-secondary">{{ translate('Total Customers') }}</div>
                    <div class="h3 fw-700 mb-0">{{ $total_customers }}</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <p>
        <span class="bg-danger d-inline-block h-10px rounded-2 w-10px"></span>
        {{ translate('This color indicates that the customer is marked as blocked.') }}
        <br>
        <span class="bg-info d-inline-block h-10px rounded-2 w-10px"></span>
            ...
            <div class="card-header row gutters-5">
                ...
                <div class="dropdown mb-2 mb-md-0">
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                        ...
                        <a class="dropdown-item" href="javascript:void(0)" onclick="bulk_export()">{{translate('Export selection')}}</a>
                    </div>
                </div>
                <div class="col-lg-2 ml-auto">
                    <select class="form-control aiz-selectpicker" name="city" onchange="sort_customers()"
                        data-selected="{{ $city }}" data-live-search="true">
                        <option value="">{{ translate('Filter by City') }}</option>
                        @foreach ($cities as $cityName)
                            <option value="{{ $cityName }}" @if($city == $cityName) selected @endif>{{ $cityName }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-lg-2">
                        ...
                        @foreach($users as $key => $user)
                            @if ($user != null)
                                <tr>
                                    ...
                                    <td>
                                        <p
                                            class="@if($user->banned == 1) text-danger @elseif($user->is_suspicious == 1) text-info @endif">
                                            @if($user->banned == 1)
...
@section('script')
    <script type="text/javascript">
        ...
        function bulk_export() {
            ...
            form.attr('action', '{{ route('bulk-customer-export') }}');
