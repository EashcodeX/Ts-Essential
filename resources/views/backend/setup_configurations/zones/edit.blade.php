            ...
            <div class="card">
                ...
                <form action="{{ route('zones.update', $zone->id) }}" method="POST">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            <label>{{ translate('Select States') }}</label>

                            <select name="state_id[]" class="aiz-selectpicker form-control" data-live-search="true"
                                multiple>
                                @foreach ($states as $state)
                                    <option value="{{ $state->id }}" @if(isset($state->zone) && $state->zone->id == $zone->id)
                                    selected @endif>
                                        {{ $state->name }}
                                    </option>
                                @endforeach
