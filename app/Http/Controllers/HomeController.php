...
class HomeController extends Controller
{
    ...
    public function trackOrder(Request $request)
    {
        if ($request->has('order_code')) {
            $order = Order::with('timelines')->where('code', $request->order_code)->first();
            if ($order != null) {
    ...
    {
        $best_selling_products = Product::orderBy('num_of_sale', 'desc')->take(18)->get();
        return view("frontend.best_selling", compact('best_selling_products'));
    }

    public function featured_products()
    {
        $featured_products = Product::where('featured', '1')->latest()->limit(12)->get();
        ...
        if ($success) {
            if ($request->ajax()) {
                return response()->json([
                    ...
                    'redirect' => route('customer-reg.verify_code', encrypt($customerVerification->id))
                ]);
            }
            return redirect()->route('customer-reg.verify_code', encrypt($customerVerification->id));
        } else {
            ...
            flash(translate('Verification code sending failed.'))->error();
            return back();
        }
    }
    ...
    public function regVerifyCodeConfirmation(Request $request)
    {
        ...
        if ($customerVerification == null) {
            ...
            flash(translate('Verification Code did not match'))->error();
            return back();
        } else {
            ...
            if ($request->ajax()) {
                return response()->json([
                    ...
                    'redirect' => route('user.registration')
                ]);
            }
            flash(translate('Verification Successful'))->success();
            return redirect()->route('user.registration')->withInput(['email' => $email, 'phone' => $phone])->with('verification_successful', true);
        }
    }
