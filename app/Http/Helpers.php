...
function getShippingCost($carts, $index, $shipping_info = '', $carrier = '')
{
    ...
    if ($shipping_type == 'flat_rate') {
        ...
        // Get zone from customer's state (via city)
        $user_zone = 0;
        if (isset($shipping_info['city_id']) && $shipping_info['city_id'] != 0) {
            $city = City::where('id', $shipping_info['city_id'])->first();
            if ($city && $city->state_id) {
                $state = State::where('id', $city->state_id)->first();
                $user_zone = $state ? $state->zone_id : 0;
            }
        }

        // Fallback to Country zone if State zone is not found
        if ($user_zone == 0 && isset($shipping_info['country_id']) && $shipping_info['country_id'] != 0) {
            $country = Country::where('id', $shipping_info['country_id'])->first();
            $user_zone = $country ? $country->zone_id : 0;
        }
...
if (!function_exists('seller_base_carrier_list')) {
    function seller_base_carrier_list($owner_id, $userId = null, $tempUserId = null, $shipping_info = null)
    {
        ...
        if (count($carts) > 0) {
            // Get zone from customer's state (via city)
            $zone = null;
            if (isset($shipping_info['city_id']) && $shipping_info['city_id']) {
                $city = City::where('id', $shipping_info['city_id'])->first();
                if ($city && $city->state_id) {
                    $state = State::where('id', $city->state_id)->first();
                    $zone = $state ? $state->zone_id : null;
                }
            }

            // Fallback to Country zone if State zone is not found
            if (($zone == 0 || $zone == null) && isset($shipping_info['country_id']) && $shipping_info['country_id']) {
                $country = Country::where('id', $shipping_info['country_id'])->first();
                $zone = $country ? $country->zone_id : null;
            }

            \Log::info('Shipping Debug: seller_base_carrier_list', [
                'owner_id' => $owner_id,
